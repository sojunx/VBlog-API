...
- **Thay đổi:**
  - Cấu hình thêm nguồn (origin) `http://localhost:5173` vào danh sách các nguồn được phép kết nối (`$allowed_origins`) bên cạnh `http://localhost:3000`.

### 📂 `public/index.php`

- **Thay đổi (Khắc phục lỗi bị chặn CORS):**
  - Di chuyển việc đăng ký `CorsMiddleware` xuống dưới cùng của danh sách middleware (sau `ErrorMiddleware` và `RoutingMiddleware`).
  - Điều này giúp đưa `CorsMiddleware` ra lớp ngoài cùng của ứng dụng (theo cơ chế LIFO của Slim 4), đảm bảo các request preflight `OPTIONS` hoặc bất kỳ phản hồi lỗi/exception nào từ hệ thống đều được bổ sung các tiêu đề CORS đầy đủ trước khi trả về cho trình duyệt của client.
